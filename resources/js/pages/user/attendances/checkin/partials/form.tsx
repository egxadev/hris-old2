...
const formSchema = z.object({
    ...
    check_in_location: z.string().min(2, {
...
export function CheckinForm({
    ...
    const getAdvancedLocation = async () => {
        try {
            ...
            form.setValue('check_in_location', locationString);
            ...
            if (locationData.isMocked) {
                setLocationError('Warning: Your location appears to be mocked. This may prevent check-in.');
            }
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        router.post('/user/attendances/checkin', values, {
            onSuccess: (page) => {
                console.log('Check-in recorded successfully.', page);
            },
            onError: (errors) => {
                console.error('Failed to check in. Errors:', errors);
            },
        });
    }
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`${className} space-y-6`}>
                ...
                <Button type="submit" disabled={isLoading || !photoTaken || !form.getValues('check_in_location')}>
