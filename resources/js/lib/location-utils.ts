...
const SUSPICIOUS_PROPS = ['FakeGPS', 'MockGPS', 'LocationFaker', 'fakegps', 'mockgps', 'isMocked', 'isFake', 'gpsSimulator'];
...
const processSecondReading = (
    ...
    if (detectSpoofedLocation(position2) || detectInconsistentReadings([position1, position2]) || detectFakeGpsApp()) {
...
const detectMockLocation = (): boolean => {
    ...
    if (userAgent.includes('emulator') || userAgent.includes('android sdk') || userAgent.includes('sdk_gphone') || userAgent.includes('x86')) {
        return true;
    }
...
const detectSpoofedLocation = (position: GeolocationPosition): boolean => {
    ...
    if (
        lat.toFixed(6) === lat.toFixed(3) + '000' ||
...
const detectInconsistentReadings = (readings: GeolocationPosition[]): boolean => {
    ...
    if (firstPos.latitude === secondPos.latitude && firstPos.longitude === secondPos.longitude) {
        return true;
    }
    ...
    if (timeDiff > 0) {
        const distance = calculateDistance(firstPos.latitude, firstPos.longitude, secondPos.latitude, secondPos.longitude);
